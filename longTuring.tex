...
of a clean interval then next time it comes in, it must extend this interval.
...
the sweeping will be complicated by a \df{zigzag} movement, using 
certain parameters \( \F,\Z \) that we set to 
 \begin{align*}
     \F    &=7,
\\   \Z   &= 14\F\beta, %?
\\  \tSim &= 1,
...
\quad  \tRebuild =3.
 \end{align*}
In its movement in direction \( \delta\in\{-1,1\}  \) the head, at every position with address 
\( \Addr\equiv\delta\tSim\pmod{\F} \)
will move backwards \( \Z \) steps (we call \df{front} the
position from which the backward movement begins).
This allows to check some consistencies, before moving forward  again.
...
\begin{align}\label{eq:PadLenDef}
 \PadLen = \F\cei{\log\V}+\Z % Change eq:PadLen?
...
\section{Healing}\label{sec:healing}
...
\begin{align*}
 \BigAlarm,
 \end{align*}
starting the rebuilding procedure, that we will specify later.
...
\begin{description}
 \item[Mark] Mark a rebuilding area consisting of \( 4Q \) cells \Pnote{?}
...
\item[Create] If there is no colony at all, create one, overriding any stragglers.
...
Then the head spends at most \( \cEsc  n^{2} \) noise-free steps inside without leaving it.
\end{lemma}
...
If it does not discover any incoordination then it escapes \( \G \)
within \( \Z n \) steps, otherwise alarm will be called.
\end{pproof} % step:clean-esc.normal
\begin{step+}{step:clean-esc.alarm}
Suppose that at some time \( t \) while the head is inside \( \G \), alarm will be called.
Then escape happens within \( O(\beta n) \) steps.
\end{step+}
\begin{pproof}
In a clean interval in the absence of noise, no alarm will be called before the end of 
the healing procedure.

If healing ends in a call to the rule \( \BigAlarm \) then, as outlined in 
...
marked cells to guarantee that no more alarm will be called before it 
ends or meets dirt.
In its first stage, the rebuilding procedure sweeps over an area of size greater
than \( Q\B \), assuring escape from interval \( \G \).
...
leading again to the escape from \( \G \).
\end{pproof} % step:clean-esc.alarm

Now consider the possibilities for when the head is found in the interval \( \G \).
...
before escaping then the rebuild procedure will take it outside \( \G \) just as seen above.
...
We will prove the case \( \abs{\G}\ge Q\B \) of the above
...
further \( \abs{D}\le n\B \), \( \abs{\G}\le m\B \).
Suppose that at some time \( t_{0} \), the head is in \( D \).
 \begin{alphenumIn}
  \item\label{i:escape.escape} As long as no more nose occurs, the 
head escapes in time \( O(n^{3}\Tu) \) from \( D \).
  \item\label{i:escape.clear} There is a constant \( c \) such that if in a noise-free time 
interval of length \( c m^{2} n\Tu \) the head does not leave the box
\( \G \) then \( \G \) becomes clean by this time.
\end{alphenumIn}
  \end{lemma}
...
Consider a clean interval \( \lint{a}{b} \) of size
\( \ge \cCleanS \B \).
Assume that an attacking event of the Attack property of a trajectory
takes place at point \( x\le b-\B \)
at the beginning time \( t \) of a noise-free time interval.
So the body of cell \( x \) is contained in \(\lint{a}{b} \)
where  \( a \) is at a distance \( \ge\cCDepth2  \) from the left edge of
the body of cell \( x \), and \( b>x+\B \).
From this time \( t \) on, we will call the interval \( \lint{a}{x+\B} \) a
\df{right-live} interval until the first time when the head returns
to the left of \( x-\cCDepth2 \B \).

Left-live intervals are defined analogously.
\end{definition}

While the interval \( \lint{a}{b} \) is right-live, 
dirt may spill over on the left of \( y \) as far as \( \cSpill \B \), that is a clean subinterval
may shrink as far as \( \lint{a}{b-\cSpill\B} \),
but this is temporary: after the return, according to the Spill Bound and Attack
properties, the whole interval \( \lint{a}{y+\B} \) will be clean.
...
further an interval \( D\subset \G \) such that
any dirt inside \( \G \) is also inside \( D \), farther than
\(  \cSpill \B \) from the edges.
Let us further have a sequence of subintervals \( J_{1},\dots,J_{k} \) of \( D \),
where \( \abs{J_{i}}\ge (\cCDepth2 +1)\B \) and
\( \abs{J_{i}\cap J_{i+1}}\le\cSpill \B \).
For each \( J_{i} \) we require that if it is to the left of the head then it is 
right-live, if it is to the right then it is left-live,
and if it contains the head then it is clean.
...
We will prove the escape lemma in a slightly stronger form.

\begin{lemma}[Partial escape]\label{lem:partial-escape}
Assume the conditions of Lemma~\ref{lem:escape},
they define a cheese at time \( t_{0} \) with box \( \G \), bulk \( D \) and no holes.
Then besides the conclusions of that lemma, the following holds:
If the head does not escape before some time
\( t\ge t_{0}+ 2i\cdot\cEsc n^{2}\Tu \) such that \( \lint{t_{0}}{t} \) is
noise-free then by that time, the sum of the sizes of holes increases by at least
\( i\B \).
\end{lemma}
  \begin{proof}
We will first prove the assertion spelled out in this lemma: deriving 
the assertions of Lemma~\ref{lem:escape} from them will be easy.

Since any dirt inside the cheese begins at least \( \cSpill \B \) removed from the edges 
of the bulk,
the Spill Bound property of a trajectory in Definition~\ref{def:traj} implies
that during the time interval considered, dirt never spills beyond the bulk.

We will define a sequence of times \( t_{0}<t_{1}<t_{2}<\dotsm<t \),
where between \( t_{i} \) and \( t_{i+1} \) always some progress will be made.

Suppose that at time \( t_{i} \) the head is in a clean part of a hole.
By Lemma~\ref{lem:clean-esc}, the head does not stay longer than
\begin{align*}
\cEsc n^{2}\Tu
 \end{align*}
in the clean part of the hole.
...
possibly with some new spilled-over dirt.

Suppose that at time \( t_{i} \) the head is not in one of the clean intervals.
Then it is either between (the clean parts of) two holes, say \( J_{p} \) and \( J_{p+1} \), 
...
Definition~\ref{def:traj} implies that the possible spilled-over dirt
...
If we had \( K\not\subseteq \G \) then the head would have escaped, 
contrary to the assumption; so we have \( K\subseteq \G \).
...
\begin{sloppypar}
The definition in Lemma~\ref{lem:clean-esc} implies
\( \cEsc n^{2} \ge \cCleanT n \).
Then \( t_{i+1}\le t_{i}+\cEsc n^{2}\Tu \) 
for each \( i \), and the sum of sizes of all the holes
increases by at least \( \B \) from \( t_{i} \) to step \( t_{i+2} \).
  \end{sloppypar}

Now~\ref{i:escape.clear}
follows immediately: if the head stays long enough in \( D \) then the holes grow
so much that \( D \) becomes clean (the total area covered by the holes is at
least half of the sum of their sizes).

To prove part~\ref{i:escape.escape}, 
let \( D(t) \) be the smallest interval such that \( G\setminus D(t) \) is clean.
Consider the two intervals whose union is \( \G\setminus D(t) \).
As soon as the head enters into one of them, that interval also becomes a hole of the cheese.
This way, the above reasoning can be easily extended.
 \end{proof}


\section{Isolated bursts}
...
containing time \( t \).
In the non-super admissible case, \( \eta^{*}(x,t)=\Bad^{*} \).
More precisely:
