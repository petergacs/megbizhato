...
The first onem called \df{recovery}, performs
...
% For controlling the details, after the interval \( \R \) 
% has been computed, the procedure uses the field \( \Rec.\Addr \) to measure
% the distance from point \( z_1 \), and a field \( \Rec.\Sweep \)
% to measure the progress, just as in the main program.

% There are corresponding \( \cRec.\Addr \) and \( \cRec.\Sweep \)
% fields in the cells.

% According to the values of \( \Rec.\Sweep \), we distinguish \df{stages},
% and introduce the \emph{pseudofield} \( \Stage \) (it is just a function of
% \( \Rec.\Sweep \)), with values
%  \begin{align*}
%    \Stage\in\{\Marking,\Planning_{j}\; (j=1,2), \Mopping_{j} (i=1,2)\}.
%  \end{align*}


% The process makes use of a number of rules:
% \( \Alarm \), \( \Purge \), \( \Mark \), \( \Plan(j) \), \( \Mop(j) \) for \( j=1,2 \), and
% \( \GiveUp \).
% Whenever we say that a rule ``checks'' something,
% it is understood that if the check fails, \( \Alarm \) is called.
% Recovery may ``fail'' for
% the following two reasons:
% the recovery interval contains cells
% marked for rebuilding procedure,
% or the patching algorithm given in Sec.~\ref{sec:patching}
% fails.
% Both of these cases indicate a ``bigger mess'', and
% they are followed by a call to
% the rule \( \GiveUp \) that we will specify later.

% From Definition~\ref{def:healthy1} it is easy to see
% that outside of islands, in an interval of \( 2\E\B \) cells, at most
% 1 small gap may exist.
% Further, the recovery interval can stretch over
...
% hence,
% condition on the single cut of Patching Lemma~\ref{lem:patching}
% is satisfied.

% In all rules of the recovery but in \( \Mop \), wherever the head steps,
% it walks on marked cells or it marks them,
% that is,  it sets \( \cRec.\Core \neq 0 \).
% The rule \( \Mop \) is devoted to unmarking.
