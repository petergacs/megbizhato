...
% For constant
% \begin{align}\label{eq:cns.dam-unit}
%    \cns{dam-unit}=6,
%  \end{align}
% during any noise-free time interval, the head does not remain longer than 
% \( 2.1\cns{dam-unit}\Tu \) in any interval of size \( \cns{dam-unit}\B \).
...
\begin{align*}
 \Alarm 
 \end{align*}
takes the state into the recovering mode.
...
\df{target}, meaning that its cells are marked as such.
...
\begin{definition}[Segments]\label{def:segments}
    An \df{(homogenous) segment} is a sequence of adjacent neighbor 
...
The \df{health} of a configuration \( \xi \) of a generalized Turing machine 
...
\item[Segments]
  The \df{base} is defined by counting back from \( \Addr \) such that
        ...
        \item In case of \( \Sweep = \TransferSw(1) \), it is the smallest interval including
          ...
          \( \front(\xi) \) on the side of the base colony.
            ...
            \item In case of \( \Sweep = \TransferSw(-1) \), it is the smallest interval including
              ...
              and the cell neighboring to \( \front(\xi) \) on the side of the base colony.

            \item If \( \TransferSw(-1) < \Sweep < \Last(\delta) \),
              then it is equal to the union of the extended base colony and the target.
            \item When \( \Sweep = \Last(\delta) \), it is the smallest interval including the future
...
\begin{definition}[Annotated configuration]\label{def:annotated-config}
  ...
  \( \cI \) is a set of intervals called \df{islands}, each of size \( \le \cns{island}\beta\B \).  %?
  They may contain an interval of damage, of size \( \le\beta\B \).

  \( \H \) is an interval containing the head called the \df{healing area}\footnote{The corresponding 
set in~\cite{burstyTuring13} called the ``distress area''.}.
  ...
  We say that an interval \( W \) is the \df{workspace} of the annotated
...
The one exception is called the \df{internal stain}.
...
\begin{definition}[Mergeable segments]
Two substantial  segments \( \lint{a_{1}}{b_{1}} \) and \( \lint{a_{2}}{b_{2}} \) will be
called \df{mergeable} if they are connected to each other with at most
...
We will sometimes just say: then the machine \df{gives up}, meaning that the
...
% is satisfied.
In the \df{planning} stages, wherever the head steps, it walks over marked cells.
In the \df{committing} stages it may set or remove marks.
...
%  \( A \time \tau \).
%  A \df{range of a sweep} \( s \) in \( \eta \) on \( A\times \tau \)
%  is the interval
...
\begin{lemma}[Damage-free escape]\label{lem:dam-free-esc}
 There is a constant
...
\end{lemma}
\begin{Proof}[Proof for the case \( n<Q \)]
...
\end{pproof} % step:dam-free-esc.normal
...
\end{pproof} % step:damage-free-esc.alarm
...
\begin{definition}[Right-live]\label{def:right-live}
...
\df{right-live} interval until the first time when the head returns
...
is called a \df{cheese of the configuration} with \( I \) called its \df{bulk} and
the intervals \( J_{i} \)  called its \df{holes}.
\Pnote{Picture?}

In a trajectory, over a certain time interval \( K \),
suppose that there is a cheese \( C(t) \) at each time \( t \).
The whole sequence of cheeses is called a \df{cheese of the trajectory}
...
We say that the head \df{escapes} the cheese if it is found % make it more  precise
...
Call an interval \( I \)  \df{special}, if it has been marked
...
We will call that the damage-elimination process arrived at a \df{standard stage}
...
\end{pproof} % step:healing.undisturbed.isolated.damage.standard
...
from the damage.
\end{prooofi} % step:healing.undisturbed.isolated.damage.start.normal
...
\end{prooofi} % step:healing.undisturbed.isolated.damage.start.healing
...
The maximum increase of the healing area is again at most \( 2\cns{island}\beta\B \).
\end{prooofi} % step:healing.undisturbed.isolated.damage.start.rebuilding
\end{pproof} % step:healing.undisturbed.isolated.damage.start
...
\begin{lemma}\label{lem:healing.undisturbed.isolated}
    Suppose that in an annotated history, following a distress-free time interval,
at some time \( t_{0} \)
an island \( I \) is encountered at a head position \( x_{0}=\xi.\pos \) at the front,
further the following holds.
\begin{enumerate}[\upshape (i)]
  \item No burst occurs during the following time interval of size
\( c_{3}\beta^{3}\Tu \), where \( c_{3} \) will be obtained from the analysis. 
\Pnote{Determine it!}
  \item There is no other island within distance \( 2\E \) nearby.
\item \( x_{0}=\chi.\front \), that is the island is encountered when the head is moving
forward in its sweep.
...
Then the following holds.
\begin{alphenum}
\item\label{i:healing.undisturbed.isolated.erase} The annotation of
the history can be extended to a certain time \( t''\le c_{3}\beta^{3}\Tu \) in such a way that
the island and the healing area disappear.
 \item\label{i:healing.undisturbed.isolated.size} 
The island and the healing area during all this is contained 
in a common interval of size \( c_{4}\beta\B \), where \( c_{4}<\Z/2 \) will be obtained
from the analysis. \Pnote{Determine it!}

 \item\label{i:healing.undisturbed.isolated.conserve} 
By the time \( t'' \),  in most cells, only the fields belonging 
to healing or rebuilding will be changed, not the fields belonging to the main simulation.
The exceptions are cells of island and possibly \( \beta \) more
cells adjacent to it.
...
\end{prooofi} % step:healing.undisturbed.isolated.alarm
...
\end{prooofi} % step:healing.undisturbed.isolated.normal
...
\end{prooofi} % step:healing.undisturbed.isolated.other
\end{Proof}
