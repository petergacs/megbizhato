...
The \df{recovery} mode tries to correct some local fault due to a couple of neighboring
...
will only become important in a recovery procedure possibly interrupted by
a burst.
...
behind when an interrupting new burst triggers new recovery.
...
The first one, called \df{recovery}, performs
...
The recovery procedure runs in \( O(\beta^{2}) \) \Pnote{was \( \beta \)}
...
As it turns out later, the rules described in this chapter remove the damage under the 
...
\subsection{Recovery addresses}\label{sec:rec-addr}

Since the number and position of some cells may change during recovery, 
we ill use the address field
 \begin{align*}
 \cRec.\Addr 
 \end{align*}
in a more complicated way than the main simulation.
It will count the number of cells 
from the left end of the recovery interval to the current cells if it is positive,
and from the right end if it is negative.
In the cells to the left of the head, \( \cRec.\Addr \) decrease to 0 towards the left, 
and in the cells to the right of the head, it increases to 0 towards the right.
The trick of two counts instead of one helps keeping track of the position,
even though the number of cells may change.
Indeed, though at the head occasionally a cell will be replaced with two cells, this does not
necessitate rewriting the values of \( \cRec.\Addr \) elsewhere.

The updating will also use the fields
\( \Rec.\Addr_{-1} \) and \( \Rec.\Addr_{1} \) of the state.
When moving in direction \( j \), field \( \Rec.\Addr_{-j} \) remembers the last
value of \( \cRec.\Addr \) set, and field \( \Rec.\Addr_{j} \) the last one seen 
before setting.
We also set \( \cRec.\Addr\gets\Rec.\Addr_{-j}+j \).
(Some modification will be needed for the case when one of the fields is 0.) \Pnote{!}

\subsection{Recovery stages}\label{sec:rec-stages}

The field \( \Rec.\Sweep \) 
measures the progress, just as \( \Sweep \) in the main program.
There is a corresponding \( \cRec.\Sweep \) field in the cells.
According to the values of \( \Rec.\Sweep \), we distinguish \df{stages}.
It is convenient to 
introduce a \df{pseudofield} \( \Stage \) (just a function of
\( \Rec.\Sweep \)), with the following possible values:
 \begin{align*}
...
the recovery interval contains cells marked for rebuilding procedure,
...
% The recovery interval can stretch over
...
% at most one pair of non-adjacent cells remains in the recovery interval,
...
Even if more would need to be accomplished at that stage, a new plannig stage
is called which decides again what to do.
This limits the possible effect of any burst interrupting recovery.
...
However, we want to make our recovery procedure eventually to work even
in the presence of \emph{damage in the islands}.
And damage can in principle kick back the recovery sweeps repeatedly, 
...
Then it sets  \( \Mode\gets\Recovering \),  \( \Stage\gets\Marking \),
\( \Rec.\Sweep \gets 1 \), and \( \Rec.\Addr = 0 \).
...
all the time: (some modification may be needed when an address is 0): \Pnote{!}
        \begin{bullets}
                  \item Check if \( \cRec.\Addr = \Rec_{j}.\Addr + j \), where
                        \( j = \pm 1 \) is the direction of the sweep.
                  \item 
                        Check if \( \cRec.\Sweep = \Rec.\Sweep -1 \).
              \end{bullets}

  Recovery starts by marking out the initial recovery area \( \R \).
...
the recovery area is surveyed to gather information needed to carry out the
...
This way, the head recovery area shrinks to the front.
