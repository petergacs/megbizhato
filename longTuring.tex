...
and an upper bound \( \Tu \) on the transition time, as well as a \df{level number} \( \lambda \),
...
The value \( 0\le L(p) \le\lambda \) shows the \df{level} of a position \( p \) on the tape.
A point is \df{clean} if  \( L(p)=\lambda \), the maximum possible level.
Informally, 
even in unclean points \( p \), the value \( L(p) \) measures the level of ``organization'' of a certain
neighborhood of \( p \).
% It is convenient to allow fractional values of \( L(p) \).
